...
const getCurrentTasks = async (req, res) => {
    ...
    if (isValid.connected){
        ...
        let query = 'SELECT ct.task_id, t.name, t.tag, tao.list_index FROM current_tasks ct INNER JOIN task t ON ct.task_id = t.task_id INNER JOIN task_order tao ON tao.current_task_id = ct.current_task_id WHERE t.user_id = $1 ORDER BY tao.list_index';
        let params = [isValid.id];
        let notPinnedSelect = await client.query(query, params);
        query = 'SELECT ct.task_id, t.name, t.tag FROM current_tasks ct INNER JOIN task t ON t.task_id = ct.task_id INNER JOIN pinned_tasks pit ON pit.current_task_id = ct.current_task_id WHERE t.user_id = $1';
